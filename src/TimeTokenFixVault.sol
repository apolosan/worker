...
contract TimeTokenFixVault is ERC4626, Minion, Ownable {

    using Math for uint256;

    uint256 private constant FIX_FEE = 10_000 - 50;

    uint256 public earned;

    address private constant WORKER_VAULT_ADDRESS = 0x17FEdeCd066A990BE5752Aa97c698477AF0E4696;
    ...
    function _afterDeposit() private {
        _earnTime();
        _produceTime();
        ITimeToken(payable(asset())).transfer(WORKER_VAULT_ADDRESS, ITimeToken(payable(asset())).balanceOf(address(this)).mulDiv(FIX_FEE, 10_000));
    }
    ...
    function mint(uint256 shares, address receiver) public override returns (uint256 assets) {
        ...
        _deposit(_msgSender(), receiver, assets, 1 ether);
        _afterDeposit();
    }
